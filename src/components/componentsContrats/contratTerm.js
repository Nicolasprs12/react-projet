...
const ContratsAttente = () => {
    ...
    const [contratsTermine, setContratsTermines] = useState([]);
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                ...
                const contratsTermine = contratData.filter(contrat => contrat.statut === "Terminé");

                // Trie les contratsEnAttente par date de création
                contratsTermine.sort((a, b) => new Date(b.dateCreation) - new Date(a.dateCreation));

                setContratsTermines(contratsTermine);
    ...
    const handleNextPage = () => {
        if ((currentPage + 1) * contractsPerPage < contratsTermine.length) {
    ...
    const displayedContracts = contratsTermine.slice(currentPage * contractsPerPage, (currentPage + 1) * contractsPerPage);

    return (
        <div>
            {contratsTermine.length > 0 ? (
                <div className='p-2'>
                    <label className="text-base lg:text-lg font-semibold mb-2 lg:mb-4">Contrats terminés</label>
                    ...
                    <div className="flex justify-between mt-4">
                        ...
                        <div className="ml-auto">
                            {((currentPage + 1) * contractsPerPage < contratsTermine.length) && (
