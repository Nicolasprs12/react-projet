...
import SuppContrat from './componentButton/suppContrat';
import PayerContrat from './componentButton/payerContrat';

const ContratsAttente = () => {
    const navigate = useNavigate();
    const [error, setError] = useState("");
    const [contratsSauve, setContratsSauvegardé] = useState([]);
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                ...
                const contratsSauve = contratData.filter(contrat => contrat.statut === "Sauvegardé");

                // Trie les contratsEnAttente par date de création
                contratsSauve.sort((a, b) => new Date(b.dateCreation) - new Date(a.dateCreation));

                setContratsSauvegardé(contratsSauve);
    ...
    const handleNextPage = () => {
        if ((currentPage + 1) * contractsPerPage < contratsSauve.length) {
    ...
    const displayedContracts = contratsSauve.slice(currentPage * contractsPerPage, (currentPage + 1) * contractsPerPage);

    return (
        <div>
            {contratsSauve.length > 0 ? (
                <div className='p-2'>
                    <label className="text-base lg:text-lg font-semibold mb-2 lg:mb-4">Contrats en Sauvegardé</label>
                        ...
                        {displayedContracts.map((contratItem, index) => (
                            <li key={index} className="bg-white shadow-lg rounded-lg p-4 lg:p-6 mb-4 lg:mb-0 lg:flex-1">
                                <div className="flex justify-between items-center">
                                    ...
                                    <PayerContrat 
                                    idContrat={contratItem.idContrat} 
                                    />
                                    <SuppContrat 
                                    idContrat={contratItem.idContrat} 
                                    />
                                ...
                                {expandedContracts[index] && (
                                    <div className="grid grid-cols-3 gap-4 pt-5">
                                        <div className="col-span-1">
                                            ...
                                            <div className="text-sm lg:text-base">Ville : {contratItem.bille}</div>
                    ...
                    <div className="flex justify-between mt-4">
                        ...
                        <div className="ml-auto">
                            {((currentPage + 1) * contractsPerPage < contratsSauve.length) && (
