...
const ContratsAttente = () => {
    ...
    const [contratsEnAttente, setContratsAttente] = useState([]);
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                ...
                const contratsEnAttente = contratData.filter(contrat => contrat.statut === "En attente");

                contratsEnAttente.sort((a, b) => new Date(b.dateCreation) - new Date(a.dateCreation));

                setContratsAttente(contratsEnAttente);
    ...
    const handleNextPage = () => {
        if ((currentPage + 1) * contractsPerPage < contratsEnAttente.length) {
    ...
    const handleValidation = async (idContrat) => {
        const token = localStorage.getItem('sessionToken');
        if (!token) {
            navigate("/accueil");
            return;
        ...
        try {
            const response = await fetch(`http://localhost/Assu_Camping2/validation`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
                },
                body: JSON.stringify({ id: idContrat }) 
            });
            if (!response.ok) {
                throw new Error('Erreur dans la validation du contrat');
            }
    ...
    const displayedContracts = contratsEnAttente.slice(currentPage * contractsPerPage, (currentPage + 1) * contractsPerPage);

    return (
        <div className="max-w-screen-xl mx-auto py-4 lg:py-8 px-2 lg:px-4" style={{ width: '90%' }}>
            {contratsEnAttente.length > 0 ? (
                <div className='p-2'>
                    <label className="text-base lg:text-lg font-semibold mb-2 lg:mb-4">Contrats en attente</label> 
                        ...
                        {displayedContracts.map((contratItem, index) => (
                            <li key={index} className="bg-white shadow-lg rounded-lg p-4 lg:p-6 mb-4 lg:mb-0 lg:flex-1">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button 
                                        onClick={() => handleValidation(contratItem.idContrat)} 
                                ...
                                {expandedContracts[index] && (
                                    <div className="grid grid-cols-3 gap-4 pt-5">
                                        ...
                                        <div className="col-span-1">
                                            ...
                                            <div className="text-sm lg:text-base">Immatriculation : {contratItem.immatriculation}</div>
                                            ...
                                            <div className="text-sm lg:text-base">Numéro identification : {contratItem.numIdentif}</div>
                                        </div>
                                        <div className="col-span-1">
                    ...
                    <div className="flex justify-between mt-4">
                        ...
                        <div className="ml-auto">
                            {((currentPage + 1) * contractsPerPage < contratsEnAttente.length) && (
